...
class App extends Component {
  ...
  moveForward() {
    ...
    if (selectedTask && selectedTask.stageId < 3) {
      const prevState = Object.assign({}, this.state.stagesTasks);
      prevState[`stage-${selectedTask.stageId}`] = prevState[
        `stage-${selectedTask.stageId}`
      ].filter((item) => {
  ...
  clickTask(e, stageId) {
    this.setState({
      selectedTask: { name: e.target.innerHTML, stageId: stageId },
    });
    console.log(stageId);
  }
    ...
    return (
      <div className="App">
        ...
        <Board
          ...
          stagesTasks={this.state.stagesTasks}
          clickTask={(e, stageId) => this.clickTask(e, stageId)}
        />
      </div>
    );
  }
